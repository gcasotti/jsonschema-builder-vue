...
const { navigateToPath, getSubSchema, setDeep, deleteDeep, renameDeep, setRequiredDeep } = _testing;
...
describe("setDeep", () => {
    it("sets a property at root level", { timeout: TIMEOUT }, () => {
        ...
        expect((result as ObjectJSONSchema).properties!.b).toEqual({ type: "number" });
        expect((result as ObjectJSONSchema).properties!.a).toEqual({ type: "string" });
    });
    ...
    it("sets a property nested in an object", { timeout: TIMEOUT }, () => {
        ...
        const person = (result as ObjectJSONSchema).properties!.person as ObjectJSONSchema;
...
describe("renameDeep", () => {
    it("renames a root-level property preserving order", { timeout: TIMEOUT }, () => {
        const root: JSONSchema = {
            type: "object",
            properties: { a: { type: "string" }, b: { type: "number" } },
            required: ["a"],
...
describe("createSchemaStore", () => {
    ...
    it("calls onChange on updateProperty", { timeout: TIMEOUT }, () => {
        ...
        expect(updated.properties!.email).toEqual({ type: "string", format: "email" });
    });
    ...
    it("getAtPath returns correct sub-schema", { timeout: TIMEOUT }, () => {
        ...
        expect((personSchema as ObjectJSONSchema).properties!.firstName).toEqual({ type: "string" });
    });

    it("produces plain objects, never reactive proxies", { timeout: TIMEOUT }, () => {
        const store = createSchemaStore(initialSchema);
