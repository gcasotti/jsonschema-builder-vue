...
describe("SchemaVisualEditor", () => {
    ...
    it("does not render AddFieldButton when read-only", { timeout: TIMEOUT }, () => {
        const wrapper = mountWithStore(SchemaVisualEditor, {
            props: { readOnly: true },
            storeSchema: { type: "object", properties: {} },
        });
    ...
    it("deletes a field via store when delete button is clicked", { timeout: TIMEOUT }, async () => {
        const schema: JSONSchema = {
            type: "object",
            properties: {
                name: { type: "string" },
        ...
        const deleteButtons = wrapper.findAll('button[aria-label]').filter(
            (btn) => btn.attributes("aria-label") === en.propertyDelete,
        );
...
describe("SchemaFieldList", () => {
    ...
    it("shows optional status for non-required fields", { timeout: TIMEOUT }, () => {
        const schema: JSONSchema = {
            type: "object",
            properties: {
